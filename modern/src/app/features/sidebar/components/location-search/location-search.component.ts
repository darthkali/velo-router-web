  ...
  template: `
    <div class="relative" #containerRef>
      ...
      <div class="relative">
        ...
        <!-- Loading Spinner with aria-live -->
        <div class="absolute inset-y-0 right-8 flex items-center pr-2" aria-live="polite" aria-atomic="true">
          @if (isLoading()) {
            <svg
      ...
      @if (isOpen() && (hasResults() || hasError())) {
        <ul
          ...
          <!-- Error State -->
          @if (hasError()) {
            <li class="relative cursor-default select-none py-2 px-3 text-red-600" role="alert">
